...
`;function at(t){const o=ot(t);return o?`
  ...
  `}function st(t,o){return`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${l(t.title)}</title><style>${B}${rt}</style></head><body>${q(t,o)}</body></html>`}function St(t,o){nt(new Blob([st(t,o)],{type:"text/html;charset=utf-8"}),`${W(t.title)}.html`)}const ct=[".field",".topology img",".topology",".overview",".project"];async function Pt(t,o){const e=document.createElement("div");e.style.cssText="position:absolute;left:0;top:0;z-index:-1;transform:translateX(-10000px);background:#fff",e.innerHTML=`<style>${B}</style>${q(t,o)}`,document.body.append(e);try{const r=e.querySelector("article");if(!r)throw new Error("无法生成导出文档，请刷新页面后重试。");await K(r,`${W(t.title)}.pdf`,{widthPx:794,imageFormat:"PNG",renderScale:2.5,pageBreakSelectors:ct,pageMarginsPx:{continuationTop:48,bottom:48},minimumPageFillRatio:.55})}finally{e.remove()}}export{st as buildExperienceDocumentHtml,St as exportExperienceHtml,Pt as exportExperiencePdf,W as safeExperienceFileName};
